...
class ManhwaForge {
  ...
  loadSettings() {
    ...
    const defaults = { groq:kc.groq||'', deepgram:kc.deepgram||'', openrouter:kc.openrouter||'', ytClientId:'', ytClientSecret:'', genre:'random', autorun:false, episodes:3, scenes:250, nextSchedule:null };
    try { const s = localStorage.getItem('manhwaforge_settings_v3'); return s ? {...defaults, ...JSON.parse(s)} : defaults; } catch { return defaults; }
  }
